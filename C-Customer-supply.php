...
if(isset($_POST['send'])){
	...
	if($v_p_name && $v_p_description){
		...
		$result = Customer::SupplyProduct($v_p_name, $v_p_description, $_SESSION['Customer_ID']);
...
?>
			...
			<div class="row">
				...
				<div class="col-sm-4 col-sm-offset-1">
					<?php
							$supplies = Customer::getSuppliedProducts($_SESSION['Customer_ID']);
							if($supplies == 0 or $supplies == 1){

								echo $msg = "<div class='alert alert-info'>You don't have supplly</div>";

							}else{
								echo "<div class='container'>";
									echo "<h4>Supplies</h4>";
										echo "<table class='table table-responsive-sm table-info table-bordered'>";
									foreach($supplies["result"] as $supply){
								?>
										<tr>
											<td><small>Supply Name: <?php echo $supply['ProductName']; ?></small></td>
											<td><small>Description: <?php echo $supply['Description']; ?></small></td>
											<td><small>Status: <?php if($supply['Status'] == 0){echo "pending";}else if($supply['Status'] == 1){echo "accepted";}?></small></td>
										</tr>
						 <?php }
						 		echo "</table>";
					 		 echo "</div>";
